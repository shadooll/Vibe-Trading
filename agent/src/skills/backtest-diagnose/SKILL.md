...
5. **Overfitting** (only when the config set `train_end`/`valid_end`): not a crash, but a statistical red flag — the strategy memorised the training segment. Symptoms (read from `run_card.json`; the nested `segments` block is stripped from the scalar-only `metrics.csv`): `unified_score` is `null` or ≤ 0, `validation_insufficient` is set (`"valid"`/`"test"`/`"overall"`), `dsr.verdict == "not_significant"`, or `segments.train.sharpe` far exceeds `segments.valid.sharpe`. Fix: simplify the strategy (fewer parameters, longer lookback windows), widen the universe or period if trades are too few, and re-validate out-of-sample — do not keep tuning on the training segment.
